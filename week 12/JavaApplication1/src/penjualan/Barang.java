...
public class Barang extends javax.swing.JFrame {
    ...
    private void tblUpdateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_tblUpdateActionPerformed
        ...
        if (recordExists(id)) {
            // Show an information message if ID exists
            JOptionPane.showMessageDialog(this, "ID bisa di bisa di update", "Info", JOptionPane.INFORMATION_MESSAGE);
            return;
        } 
         String kode = txtKode.getText().toString().trim();
        String nama = txtNama.getText();
    ...
    private void tblDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_tblDeleteActionPerformed
        // TODO add your handling code here:
     String id = txtId.getText().trim();
     String barangK = txtKode.getText().trim();
        ...
        if (recordExists(id)) {
            JOptionPane.showMessageDialog(this, "ID ini telah di hapus", "Error", JOptionPane.INFORMATION_MESSAGE);
            deleteid(id);
            showtable();
            clear();
            return;
        ...
        if (!recordExists(id)) {
            JOptionPane.showMessageDialog(this, "ID tidak ditemukan dalam database", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
    ...
    try {
        ...
        String checkSql = "SELECT COUNT(*) FROM barang WHERE kd_brg = ?";
        ...
        if (count > 0) {
            // Kode already exists, show a message dialog
            JOptionPane.showMessageDialog(this, "Kode sudah ada dalam database", "Error", JOptionPane.ERROR_MESSAGE);
        } else {
            // Kode does not exist, insert the record
            String insertSql = "INSERT INTO barang (kd_brg, nm_brg, satuan, stok, stokmin) VALUES (?,?,?,?,?)";
            PreparedStatement insertStmt = conn.prepareStatement(insertSql);
            insertStmt.setString(1, kode);
            insertStmt.setString(2, nama);
            insertStmt.setString(3, satuan);
            insertStmt.setInt(4, stok);
            insertStmt.setInt(5, stokmin);
    ...
    public boolean recordExists(String id) {
        try {
            ...
            String checkSql = "SELECT COUNT(*) FROM barang WHERE id = ?";
            PreparedStatement checkStmt = conn.prepareStatement(checkSql);
    ...
    public void update(String kode, String nama, String satuan, int stok, int stokmin) {
        try {
            ...
            // Use kode instead of id as the parameter for the WHERE clause
            String updateSql = "UPDATE barang SET kd_brg = ?, nm_brg = ?, satuan = ?, stok = ?, stokmin = ? WHERE kd_brg = ?";
            PreparedStatement updateStmt = conn.prepareStatement(updateSql);
            updateStmt.setString(1, kode);
            updateStmt.setString(2, nama);
            updateStmt.setString(3, satuan);
            updateStmt.setInt(4, stok);
            updateStmt.setInt(5, stokmin);
    ...
    public void delete(String barangK) {
    try {
        Class.forName(JDBC_DRIVER);
        conn = DriverManager.getConnection(DB_URL, USER, PASS);
        
        String deleteSql = "DELETE FROM barang WHERE kd_brg = ?";
        PreparedStatement deleteStmk = conn.prepareStatement(deleteSql);
        deleteStmk.setString(1, barangK);

        deleteStmk.executeUpdate();
